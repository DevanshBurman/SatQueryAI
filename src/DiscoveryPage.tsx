...
export default function DiscoveryPage({ proceed, back }: { proceed: (scenes: CatalogScene[]) => void; back: () => void }) {
  ...
  const toggleSelected = (scene: CatalogScene) => setSelected(items => items.some(s => s.id === scene.id) ? items.filter(s => s.id !== scene.id) : [...items, scene])
  const loadExamples = () => {
  ...
  const locate = (event: React.FormEvent) => {
    ...
    map.current?.flyTo({ center: [parts[1],parts[0]], zoom: 10, duration: 600 }); setNotice('Location updated. Search this area to refresh observations.')
  }
  ...
  return <section className="discovery" aria-label="Imagery discovery">
    <aside className="discovery-panel">
      ...
      <div className="discovery-list">
        ...
        {scenes.filter(scene => mode === 'all' || scene.mode === mode).map(scene => <article className={active?.id === scene.id ? 'is-active' : ''} key={scene.id}>
          <button className="discovery-scene" onClick={() => focusScene(scene)} aria-label={`Inspect ${scene.source} ${scene.date}`}><span className="discovery-thumbnail">{scene.thumbnail ? <img src={scene.thumbnail} alt="Provider scene thumbnail" onError={e => { e.currentTarget.style.display = 'none' }} /> : <Layers3 size={25}/>}</span><span><small>{scene.mode === 'sar' ? 'RADAR / SAR' : 'OPTICAL'}</small><b>{scene.source}</b><span>{scene.date} · {scene.resolution_m} m</span>{scene.cloud !== null && <span>Cloud cover {Math.round(scene.cloud)}%</span>}</span></button>
    ...
    <div className="discovery-canvas">
      ...
      {active && <div className="discovery-inspector">
        ...
        {active.thumbnail && <img className="discovery-provider-preview" src={active.thumbnail} alt="Provider preview, not a georeferenced map overlay"/>}
