...
function FloodShape({className=''}:{className?:string}){return <svg className={'sq-boundary '+className} viewBox="0 0 1000 700" preserveAspectRatio="none" aria-hidden="true"><path pathLength="1" d="M568 165L585 153 610 175 614 198 645 205 654 234 680 246 672 270 707 290 727 279 760 294 776 325 811 330 799 352 832 369 824 391 859 421 846 444 888 472 872 493 901 527 881 544 850 525 823 537 797 510 768 502 751 469 725 467 702 445 677 429 663 402 643 397 630 366 611 354 607 328 587 306 594 282 575 264 582 238 564 216Z"/><path className="sq-reference-line" d="M540 150L925 527 863 570 532 223Z"/></svg>}
...
export default function LandingExperience({enter}:{enter:()=>void}){
  ...
  return <main className="sq-site" ref={root}>
    ...
    <section className="sq-hero" aria-labelledby="sq-title">
      <div className="sq-earth"><img src="/hero-floodplain.png" alt="Illustrative satellite-style view of a braided river and its floodplain"/><FloodShape/></div><div className="sq-hero-shade"/>
      ...
      <div className="sq-place"><MapPin/><span>Supaul<small>Bihar</small></span></div><div className="sq-area-tag"><MapPin/><span>Inundation study<small>Selected floodplain</small></span></div><span className="sq-river-name">Kosi River ↗</span>
      <div className="sq-legend"><div><i/>Illustrative flood extent</div><div><i/>Reference area</div><div className="sq-scale"><span>0</span><span>2.5</span><span>5 km</span><b>↑<small>N</small></b></div><small>Concept imagery · not an observed flood product</small></div>
    ...
    {playing&&<div className="sq-demo-backdrop" role="dialog" aria-modal="true" aria-label="Product demo"><div className="sq-demo"><button className="sq-demo-close" aria-label="Close demo" onClick={()=>setPlaying(false)}><X/></button><div className="sq-demo-visual"><img src="/hero-floodplain.png" alt="Illustrated floodplain workflow"/><FloodShape/><span>{chapters[tour].label}</span></div><div className="sq-demo-copy"><span>Interactive product tour · {tour+1} / 4</span><h2>{chapters[tour].title}</h2><p>{chapters[tour].copy}</p><div>{chapters.map((c,i)=><button key={c.title} aria-label={c.label} className={tour===i?'active':''} onClick={()=>setTour(i)}/>)}</div><button className="sq-primary" onClick={enter}>Explore the prototype <ChevronRight/></button></div></div></div>}
  </main>
}
